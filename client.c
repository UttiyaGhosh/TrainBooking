...
int do_admin_action(int sock, int opt){
	switch(opt){
		...
		case 2:{
			...
			write(sock, &opt, sizeof(opt));
			view_trains(sock);
			printf("Enter the train id to delete: "); scanf("%d", &no_of_trains);
...
if(no_of_trains!=0){
		...
		case 5: {
			...
			read(sock, &opt, sizeof(opt));
			if(opt == 5) printf("Operation succeeded\n");
			while(getchar()!='\n');
			getchar();
			return opt;
		}
		...
		case 7:{
			...
			printf("---------------------------------------------\n");
			printf("|    UserID|      Type|  UserName|  PASSWORD|\n");
			printf("---------------------------------------------\n");
			while(entries--){
				int id, type;
...
void view_trains(int sock){
	...
	printf("No train found\n");
		...
		printf("---------------------------------------------\n");
		printf("|   TrainID| Train No.|Train Name|  AV Seats|\n");
		printf("---------------------------------------------\n");
		while(entries--)
		{
			int id, tno, av_seats;
...
void view_booking(int sock, int all){
	...
	if(entries == 0) printf("No records found.\n");
		...
		printf("-------------------------------------------------------------------------------\n");
		printf("|Booking ID|1st Ticket|Last Ticket|     Train|   User ID| User Type|    STATUS|\n");
printf("-------------------------------------------------------------------------------\n");
	}
	else{
		printf("----------------------------------------------\n");
		printf("|Booking ID|1st Ticket|Last Ticket|     Train|\n");
		printf("----------------------------------------------\n");
	}
	while(entries--){
